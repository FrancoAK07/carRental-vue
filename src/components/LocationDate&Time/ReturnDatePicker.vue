<template>
	<div class="datepicker-container p-0 w-75 border-0">
		<input type="text" class="border w-100 px-2 rounded-start position-relative z-0" placeholder="Return Date" ref="returnDateInput" @click="displayDates()" />
		...
		<div class="datepicker position-absolute bg-dark-subtle mt-1 start-0 p-3 rounded z-3" ref="datePicker" v-show="showReturnDatePicker">
			...
			<div class="datepicker-footer row w-100 mx-auto">
				<button class="col cancel btn btn-secondary me-5" @click="showReturnDatePicker = false">Cancel</button>
...
<script setup>
	...
	let showReturnDatePicker = ref(false);
	let returnDateInput = ref(null);
	let monthInput = ref(null);
	let yearInput = ref(null);
	let dates = ref(null);
	let datePicker = ref(null);

	const emit = defineEmits(["returnDate"]);

	const setDate = () => {
		returnDateInput.value.value = selectedDate.toLocaleDateString("en-US", {
			year: "numeric",
			month: "2-digit",
			day: "2-digit",
		});
	...
	const displayDates = () => {
		showReturnDatePicker.value = true;
	...
	window.addEventListener("click", (e) => {
		if (showReturnDatePicker.value) {
			if (!datePicker.value.contains(e.target) && !returnDateInput.value.contains(e.target)) {
				showReturnDatePicker.value = false;
			}
		}
	});
